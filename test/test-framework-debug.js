...
ON('ready', function() {
	...
	assert.ok(CONF.array.length === 4, 'Problem with config sub types.');
...
F.onError = function(error, name, uri) {
...
ON('load', function() {
	...
	MERGE('/mergepackage.js', '@testpackage/test.js');
	MERGE('/mergedirectory.js', '~' + PATH.public('js') + '*.js');
...
HTTP('debug', { port: 8001, thread: 'users' });
